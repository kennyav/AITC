...
const RelayProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  // immediately set the relay to the first one in the list
  ...
  // publish event to relays on nostr
  const publish = async (
    relays: string[],
    event: any,
    onOk: () => void,
  ...
  // subscribe to events on relays
  const subscribe = async (
    relays: string[], // list of relays that you wnat
    filter: any, // filter for anything that you want to get back
    onEvent: (event: any) => void, // do something when you get a particular event back
    onEOSE: () => void
  ) => {
    for (const url of relays) {
      const relay = await connect(url);
